...
public class JdbcQuoteRepositoryUsingJson implements QuoteRepository {
    ...
    private Quote mapRow(ResultSet rs, int rowNum) {
        try {
            ...
            var subjectsIterator = new ObjectMapper().readValue(subjectsJsonString, String[].class);
            var subjects = Arrays.stream(subjectsIterator).collect(ImmutableList.toImmutableList());
